...
def tier_sort(nodes, sizes, children, reverse_nodes=False, f_bounds = False):
    tiers = {}
    node_tier = {}
    sort_nodes = []
    lvl = 0
    ...
    for node in sort_nodes:
        ...
        if sort_children[node] is not None:
            tmp_node_tier = dict(sorted(tmp_node_tier.items(), key=lambda item: item[1]))
...
def define_m(n, children, nodes, parents):
    ...
    for i in nodes:
        for j in nodes:
            ...
            if i != j:
                m_subj.append('m_' + str(i) + '_' + str(j) + ' + ' + 'm_' + str(j) + '_' + str(i) + ' = 1')
    ...
    for i in nodes:
        ...
        for j in nodes:
            ...
            if int(i) < int(j):
                ...
                s_subj_4.append(
                    's_' + str(i) + ' + ' + str(n) + ' m_' + str(i) + '_' + str(j) + ' - s_' + str(j) + ' <= ' + str(
                        n - 1))
                # s_subj_5.append(f's_{j} - s_{i} - {n} m_{i}_{j} <= -1')
                s_subj_5.append(
                    's_' + str(j) + ' - s_' + str(i) + ' - ' + str(n) + ' m_' + str(i) + '_' + str(j) + ' <= -1')
        s1 += ' = 0'
        s_subj_1.append(s1)
...
def parse(type, path_inp, sort='default'):
    ...
    for file_name in files:
        ...
        n = len(nodes)
        path = "solver_inputs/" + file_name + "_" + sort + "_input.lp"
